...
`.stats`  |  Shows some basic stats for CynBot.
`.dysyd`  |  Shows some basic stats for CynBot.
...
`;permrole`, `;pr`  |  Sets a role which can change permissions. Or supply no parameters to find out the current one. Default one is 'CynBot'.
...
`@BotName uptime`  |  Shows how long CynBot has been running for.
...
`$$$`  |  Check how much CynBotFlowers you have.
`$give`  |  Give someone a certain amount of CynBotFlowers
...
`>rps`  |  Play a game of rocket paperclip scissors with CynBot. |  >rps scissors
...
`>heal`  |  Heals someone. Revives those that fainted. Costs a CynBotFlower  | >revive @someone
`>type`  |  Get the poketype of the target. |  >type @someone
`>settype`  |  Set your poketype. Costs a CynBotFlower. |  >settype fire
